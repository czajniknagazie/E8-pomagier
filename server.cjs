...
const UPLOADS_DIR = process.env.UPLOADS_DIR || "./uploads"; // To będzie na Dysku Stałym Railway
const saltRounds = 10;

// ZMIANA: Konfiguracja połączenia z PostgreSQL (automatycznie użyje DATABASE_URL z Railway)
const pool = new Pool({
    connectionString: process.env.DATABASE_URL,
    ssl: {
        rejectUnauthorized: false // Wymagane na darmowym planie Render/Railway
    }
});

// ZMIANA: Funkcja do inicjalizacji bazy danych (tworzenia tabel)
async function initializeDatabase() {
    ...
    try {
        ...
        // ZMIANA: Używamy składni PostgreSQL (SERIAL, JSONB, "user" w cudzysłowach)
        await client.query(`
        ...
        // Używamy ON CONFLICT, aby zaktualizować hasło admina, jeśli już istnieje
        await client.query(
        ...
        console.log("Konto administratora ('admin') zweryfikowane/utworzone.");
        
        client.release(); // Zwróć klienta do puli
    } catch (err) {
...
// --- JWT & Authentication ---
// (Ta sekcja jest poprawna i pozostaje bez zmian)
...
// --- Login Endpoints ---
// ZMIANA: Wszystkie zapytania są teraz asynchroniczne (async/await) i używają pool.query
...
app.get("/api/tasks", auth(), async (req, res) => {
    ...
    const params = [];
    if (search) {
        queryText += ' WHERE id::text LIKE $1 OR arkusz LIKE $1'; // $1 dla parametru
        params.push(`%${search}%`);
    }
...
app.put("/api/tasks/:id", auth("admin"), async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            ...
            [odpowiedz, Number(punkty), opcjeJson, id]
        );
...
app.get("/api/tasks/random", auth(), async (req, res) => {
    ...
    let queryText;
    const params = [req.user.name, mode]; // $1 = user.name, $2 = mode
...
app.post("/api/solved", auth(), async (req, res) => {
  ...
  try {
    // ZMIANA: Składnia ON CONFLICT dla PostgreSQL
    await pool.query(
...
app.post("/api/tasks/bulk", auth("admin"), async (req, res) => {
  ...
  try {
    ...
    for (const t of tasks) {
        // Konwersja opcji na JSONB
        const opcjeJson = t.opcje ? JSON.stringify(t.opcje) : null;
        await client.query(
            `INSERT INTO tasks (type, tresc, odpowiedz, opcje, punkty) VALUES ($1, $2, $3, $4, $5)`,
            [t.type, t.tresc, t.odpowiedz, opcjeJson, Number(t.punkty) || 1]
        );
    }
...
app.delete("/api/tasks/:id", auth("admin"), async (req, res) => {
    const { id } = req.params;
    try {
        // Musimy usunąć powiązane wpisy w 'solved' zanim usuniemy zadanie (jeśli masz klucze obce)
...
app.get("/api/exams/:id", auth(), async (req, res) => {
  ...
  // ZMIANA: Składnia PostgreSQL dla listy ID (ANY)
  const tasksResult = await pool.query(`SELECT * FROM tasks WHERE id = ANY($1::int[])`, [ids]);
  
  // Sortowanie zadań w kolejności, w jakiej są w `ids`
  const tasksMap = new Map(tasksResult.rows.map(t => [t.id, t]));
  const sortedTasks = ids.map(id => tasksMap.get(id)).filter(Boolean); // Filtruj null, jeśli ID nie istnieje
...
app.delete("/api/exams/:id", auth("admin"), async (req, res) => {
    const { id } = req.params;
    try {
        // Usuń powiązane wyniki egzaminów (jeśli masz klucze obce)
...
// ZMIANA: Uruchom serwer DOPIERO po inicjalizacji bazy danych
initializeDatabase().then(() => {
    app.listen(PORT, () => {
        console.log(`Server listening on port ${PORT}`);
    });
}).catch(err => {
    console.error("Nie udało się uruchomić serwera z powodu błędu bazy danych:", err);
});